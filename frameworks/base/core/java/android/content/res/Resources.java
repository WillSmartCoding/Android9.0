...
/**
 * 类来访问应用程序的资源。它位于应用程序的资产管理器之上（可通过{@link#getAssets}）并提供一个高级API，用于从资产中获取类型化数据。
 *
 * <p>Android资源系统跟踪与应用程序相关联的所有非代码资产。
 * 您可以使用这个类来访问应用程序的资源。
 * 您通常可以获得{@link android.content.res.Resources}实例
 * 与{@link android.content.Context#getResources getResources()}</p>
 *
 * <p>androidsdk工具在构建时将应用程序的资源编译为应用程序二进制文件。
 * 要使用资源，必须将其正确安装在源代码树中（在项目的{@code res/}目录中）并构建应用程序。
 * 作为构建过程的一部分，SDK工具为每个资源生成符号，您可以在应用程序代码中使用这些符号来访问资源。</p>
 *
 * <p>使用应用程序资源可以在不修改代码的情况下轻松地更新应用程序的各种特性，
 * 而且通过提供一组替代资源，您可以针对各种设备配置（例如针对不同的语言和屏幕大小）优化应用程序。
 * 这是开发在不同类型设备上兼容的Android应用程序的一个重要方面。</p>
 *
 * <p>有关使用资源的更多信息，请参阅关于<a href="{@docRoot}guide/topics/resources/index.html">应用程序资源</a></p>
 */
public class Resources {
     ...
     * 此解析器的功能有限，尤其是不能更改其输入，并且只有高级事件可用。
     * 
     * <p>这个函数实际上是一个简单的包装器，用于使用布局资源调用{@link #getXml}。
     * 
     * @param id 由aapt工具生成的所需资源标识符。此整数对包、类型和资源项进行编码。值0是无效的标识符。
     *
     * @throws NotFoundException Throws NotFoundException if the given ID does not exist.
     * 
     * @return 一个新的解析器对象，通过它可以读取XML数据。
     *         
     * @see #getXml
     */
    @NonNull
     ...
     * 
     * <p>这里返回的XmlPullParser实现具有一些有限的功能。尤其是，
     * 您不能更改其输入，并且只有高级解析事件可用（因为文档是在构建时为您预解析的，这涉及合并文本和剥离注释）.
     * 
     * @param id The desired resource identifier, as generated by the aapt
     *           tool. This integer encodes the package, type, and resource
     *           entry. The value 0 is an invalid identifier.
